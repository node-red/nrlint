    ...
    static parseFlow(flowarr) {
        ...
        flowarr.forEach((elem) => {
            if (elem.type !== 'subflow' && elem.type !== 'tab') { // regular nodes (incl. subflow instances)
                ...
                if ('x' in elem && 'y' in elem) { // a node on UI
        ...
        flowarr.forEach((elem) => {
            if (elem.type !== 'subflow' && elem.type !== 'tab') {
                ...
                props.forEach((prop) => {
                    ...
                    prop !== 'type' &&
                    prop !== '_users' &&
                    elem[prop] in flowreg.configs) { // Using global config properties
                ...
                if (elem.z && ! flowreg.subflows[elem.z]) { // this node belong to the subflows
                    ...
                    if (addedTabs[elem.z]) {
                        if ('x' in elem && 'y' in elem) { // regular node
                            addedTabs[elem.z].nodes[elem.id] = elem;
                        } else { // config node
                            addedTabs[elem.z].configs[elem.id] = elem;
                        }
                    }
                }
            }
        ...
        for (const cid of configIds) {
            ...
            Object.assign(c, flowreg.configs[cid]);
            fs.configs.push(c);
        }
