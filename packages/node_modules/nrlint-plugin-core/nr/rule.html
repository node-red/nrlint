 ...
 **/
 ...
 function checkSeverity(conf, dflt) {
    if ('severity' in conf) {
        if (conf.severity == "error") {
            return "error";
        } else if (conf.severity == "warn") {
            return "warn";
        } else {
            return dflt;
        }
 ...
 * @param {*} conf configuration for this rule. {name: "loop", sevirity: "warn"}
 * @param {*} cxt context
...
function checkLoop(afs, conf, cxt) {
    ...
    const severity = checkSeverity(conf, "warn");

    return {
        context: cxt,
        result: loops.map(l => {return {rule: "core", ids: [...new Set(l)], severity: severity, name: "loop", message: "possible infinite loop detected"};})
    };
}
...
function checkHttpInResp(afs, conf, cxt) {
    ...
    const danglingHttpIns = afs.getAllNodesArray()
        ...
        .map(e => ({rule:"core", ids: [e.id], name: "dangling-http-in", severity: severity, message: "dangling http-in node"}));
    ...
    const danglingHttpResponses = afs.getAllNodesArray()
        ...
        .map(e => ({rule:"core", ids: [e.id], name: "dangling-http-resp", severity: severity, message: "dangling http-response node"}));
...
function checkFlowSize(afs, conf, cxt) {
    ...
    flows.forEach((flow) => {
        ...
        if (nodes.length > maxSize) {
            result.push({rule: "core", ids: [flow.id], name: "flowsize", severity: severity, message: "too large flow size" } );
        }
    });
...
function checkNoFuncName(afs, conf, cxt) {
    ...
    var verified = funcs
        ...
        .map(function (e){
            return {rule:"core", ids: [e.id], severity: severity, name: "no-func-name", message: "function node has no name"};
        });
...
RED.plugins.registerPlugin('nrlint-plugin-core', {
    type: 'nrlint-rule',
    onadd: function() {
        //console.log('nrlint-plugin-core:onadd called');
    },
    onremove: function() {
        //console.log('nrlint-plugin-core:onremove called')
    },
    check: check,
    id: 'nrlint-plugin-core' // workaround
});
})();
</script>
